...
void PCAPDataSource::start()
{
    ...
    for(;;)
    {
        ...
        if (m_ProcessedCount && m_Current)
        {
            m_Sink->notify(m_Current, m_ProcessedCount);
            m_Current = nullptr;
            m_ProcessedCount = 0;
        }
        ...
        {
            if (status == 0 && isFile() && isRewinding())
            {
                pcap_close(m_Handle);
                char errbuf[PCAP_ERRBUF_SIZE];
                m_Handle = pcap_open_offline(m_Source.c_str(), errbuf);
                if (!m_Handle)
                    THROW_EXCEPTION("could not open the specified file: " + m_Source + ": " + errbuf);
                resetFilter();
            }
            else
                break;
...
void PCAPDataSource::openDefaultDevice(size_t snaplen)
{
    ...
    auto selected_device = alldevs;
    for(auto iter = alldevs; iter; iter = iter->next)
    {
        if(iter->description && strstr(iter->description, "Ethernet"))
        {
            selected_device = iter;
            break;
        }
    }

    m_Source = selected_device->name;
...
void PCAPDataSource::resetFilter()
{
    ...
    if (pcap_compile(m_Handle, &m_Filter, m_FilterExpression.c_str(), 0, mask) < 0)
        THROW_EXCEPTION("could not parse filter \"" + m_FilterExpression + "\": " + pcap_geterr(m_Handle));
    if (pcap_setfilter(m_Handle, &m_Filter) < 0)
        THROW_EXCEPTION("could not install filter \"" + m_FilterExpression + "\": " + pcap_geterr(m_Handle));
}
...
void PCAPDataSource::handler(u_char* args, const pcap_pkthdr* header, const u_char* packet)
{
    ...
    src.m_ProcessedCount++;

    packet_info->init(pcap_datalink(src.m_Handle), Time(header->ts), header->caplen, header->len);
    assert(count);
}
