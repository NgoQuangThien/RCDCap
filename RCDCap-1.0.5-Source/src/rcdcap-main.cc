...
{
    double res;
    if((res = b / (1024.0*1024.0*1024.0*1024.0)) > 1.0)
        os << res << "TB";
    else if((res = b / (1024.0*1024.0*1024.0)) > 1.0)
        os << res << "GB";
    else if((res = b / (1024.0*1024.0)) > 1.0)
        os << res << "MB";
    else if((res = b / 1024.0) > 1.0)
        os << res << "KB";
    else
        os << b << "B";
    return os;
        ...
        m_Signals(RCDCap::make_unique<boost::asio::signal_set>(m_IOService, SIGINT, SIGTERM))
{
#ifdef _WIN32
#ifdef _WIN64
	SetDllDirectory("C:\\Windows\\SysWOW64\\Npcap\\");
#else
    ...
    m_Description.add_options()
		...
		("load-plugins", "Load the specified plug-in files", cxxopts::value<std::vector<std::string>>())
        ("disable-vlan-tag", "Disable the insertion of the 802.1Q VLAN Tag inside the decapsulated packet by some processors")
        ("H,hp-erm-server", "Run the application as a HP ERM destination host", cxxopts::value<size_t>()->implicit_value("7932"))
        ("thread-pinning", "Set the threads to run on the 'same' cpu, 'different' or 'disable' it and let the scheduler decide", cxxopts::value<std::string>()->default_value("disable"))
        ("capture-mode", "Specify whether the capturing process should run separately or compete for resources with the rest of the application", cxxopts::value<std::string>()->default_value("auto"))
        ("worker-threads", "Force the application to spawn a given amount of worker threads", cxxopts::value<size_t>())
        ("dummy", "Ignore any other output options and just count packets")
        ("t,tap-device", "Output the raw packets to a TAP device", cxxopts::value<std::string>())
        ("tap-dev-addr", "Set the address of the TAP device to the specified (this option is valid only if --tap-device is specified)",
                         cxxopts::value<std::string>())
        ("tap-persist", "Set the TAP device to persistent mode")
        ("r,read-binary", "Read the raw packets from the specified file or stdin, if ``-'' is passed",
                          cxxopts::value<std::string>())
        ("R,rewind", "Rewind after reaching end of file")
        ("l,list-devices", "List the available devices")
        ("o,write-text", "Write some information about the captured packets in text format to the specified"
                         "file or stdout, if ``-'' is passed (this option is used by default when no output"
                         "method is specified)",
                         cxxopts::value<std::string>())
        ("w,write-binary", "Write the raw packets to the specified file or stdout, if ``-'' is passed",
                           cxxopts::value<std::string>())
        ("D,discard-packets", "Discard all packets after processing")
        ("I,inject", "Write the raw packets to the specified Ethernet device",
                     cxxopts::value<std::string>())
        ("i,interface", "Set the network interface to listen on",
                        cxxopts::value<std::string>())
        ("b,buffer-size", "Set the internal buffer to the specified size in the specified unit",
                          cxxopts::value<Bytes>()->default_value("100MB"))
        ("s,snaplen", "Set the snapshot length in bytes",
                      cxxopts::value<size_t>()->default_value("1518"))
#ifdef HAS_PF_RING
...
#endif
        ("hp-erm", "Enable the HP ERM decapsulating processor", cxxopts::value<uint16>())
        ("erspan", "Enable the ERSPAN decapsulating processor")
        ("y,ignore-incomplete", "Ignore incomplete packets")
        ("f,force-incomplete", "Ignore incomplete packets")
        ("d,daemonize", "Launch RCDCap as separate ")
        ("T,timeout", "Specifies how much the application should wait in milliseconds before breaking"
                      "the capture process to send the packets to the next element in the pipeline", 
                      cxxopts::value<size_t>()->default_value("1"))
        ("B,burst-size", "How many packets should be processed at a time",
                         cxxopts::value<size_t>()->default_value("10"))
        ("expression", "Set the PCAP filter expression", cxxopts::value<std::vector<std::string>>())
...
void RCDCapApplication::initPlugins(cxxopts::ParseResult& vm)
{
...
{
#ifdef _WIN32
    std::cout << "daemonization is not supported on windows";
#else
...
{
    ThreadGroup                             threads;
    auto interm_argc = argc;
    auto interm_argv = argv;
    try
    {
    {
		cxxopts::ParseResult vm = m_Description.allow_unrecognised_options().parse(interm_argc, interm_argv);
		initPlugins(vm);
    }

	cxxopts::ParseResult vm = m_Description.parse(argc, argv);
	auto expr_count = vm.count("expression");
    if(expr_count)
    {
        auto exprs = vm["expression"].as<std::vector<std::string>>();
        for(auto& expr : exprs)
            m_Expression += expr + " ";
    }
...
{
    std::cout << m_Description.help() << std::endl;
}
...
void RCDCapApplication::listDevices()
{
    char errbuf[PCAP_ERRBUF_SIZE];
    pcap_if_t *alldevs;
    int ret = pcap_findalldevs(&alldevs, errbuf);
    if (ret == PCAP_ERROR || !alldevs)
    ...
    std::cout << "Devices:\n";
    size_t idx = 1;
    for (auto iter = alldevs; iter; iter = iter->next, idx++)
    {
...
void RCDCapApplication::spawnWorkerThreads(cxxopts::ParseResult& vm, ThreadGroup& threads)
{
...
#undef max
        ...
        ++i, cpu = (cpu + 1u) % (num_cpu - offset_cpu))
    {
        ...
        threads.push_back(std::thread([this, &io_service]() {
            try
            {
                io_service.run();
            }
            ...
            {
                std::cerr << "rcdcap: error: worker crashed: boost::system::system_error: " << e.what() << std::endl;
                this->workerCrash();
            }
            catch (const RCDCap::Exception& e)
            {
                std::cerr << "rcdcap: error: worker crashed: " << e.what() << std::endl;
                this->workerCrash();
            }
            catch (const std::exception& e)
            {
                std::cerr << "rcdcap: error: worker crashed: std::exception: " << e.what() << std::endl;
                this->workerCrash();
            }
            catch (...)
            {
                std::cerr << "rcdcap: error: worker crashed: unknown exception." << std::endl;
                this->workerCrash();
            }
        }));
...
void RCDCapApplication::initSource(cxxopts::ParseResult& vm)
{
    ...
    else if(rc == 1)
    {
        ...
        auto& src = reinterpret_cast<PCAPDataSource&>(*m_Source);
        src.openFile(vm["read-binary"].as<std::string>());
        src.setRewind(vm["rewind"].as<bool>());
    }
    else
    {
        operation = "listening on ";
...
void RCDCapApplication::initSink(cxxopts::ParseResult& vm, const SourcePtr& _last)
{
    uint32 ip = 0, flags = 0;
...
void RCDCapApplication::initPipeline(cxxopts::ParseResult& vm, SourcePtr& _last)
{
...
void RCDCapApplication::workerCrash()
{
    m_ProgramStatus = EXIT_FAILURE;
    terminate();
}
