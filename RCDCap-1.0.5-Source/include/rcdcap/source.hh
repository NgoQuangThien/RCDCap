...
public:
    ...
    */ 
    ...
    virtual void setRewind(bool value) {}
...
{
    std::string                         m_FilterExpression; //!< Filter expression string.
    ...
    size_t                              m_ProcessedCount = 0; //!< Avoids weird bugs in some PCAP implementations.
...
public:
    ...
    virtual void setRewind(bool value) override
    {
        if(value)
            m_Flags |= PCAP_Rewind;
        else
    ...
    virtual bool isRewinding() const { return m_Flags & PCAP_Rewind; }
...
private:
    ...
    //! Reinitialize the filter expression
    void resetFilter();
