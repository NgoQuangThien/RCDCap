...
class RCDCapApplication
{
    ...
    cxxopts::Options		                    m_Description;  //!< The description of the command line arguments that this application accepts
    ...
    int m_ProgramStatus = EXIT_SUCCESS;
public:
    //! Constructor.
    RCDCapApplication();
    ...
    //! Lists the available Ethernet devices
    void listDevices();
    
    //! Terminates the application prematurely.
    void terminate();

    //! Handles a worker crash.
    void workerCrash();

    //! Reports program status.
    int status() const { return m_ProgramStatus; }
...
private:
     ...
     */
    void initSource(cxxopts::ParseResult& vm);
     ...
     */
    void initSink(cxxopts::ParseResult& vm, const SourcePtr& _last);
     ...
     */
    void initPipeline(cxxopts::ParseResult& vm, SourcePtr& _last);
     ...
     */
    void spawnWorkerThreads(cxxopts::ParseResult& vm, ThreadGroup& threads);
     ...
     *  \param vm        a map that contains all of the parsed variables that were passed when executing the application.
     */
    void initPlugins(cxxopts::ParseResult& vm);
