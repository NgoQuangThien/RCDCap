...
public:
    ...
    //! Sets checksum present field
    void setCheksumPresent(bool enabled) { return m_Flags.set<BF_CHECKSUM_BIT>(enabled); }

    //! Sets sequence number present field
    void setSeqNumPresent(bool enabled) { return m_Flags.set<BF_SEQUENCE_NUMBER_BIT>(enabled); }

    //! Sets version field
    void setVersion(uint8 version) { return m_Flags.set<BF_VERSION>(version); }
