...
{
#ifdef _WIN32
    typedef boost::asio::windows::stream_handle StreamDescriptor;
#else
    typedef boost::asio::posix::stream_descriptor StreamDescriptor;
#endif

    StreamDescriptor   m_SD;       //!< The stream descriptor that is representing the TAP device.
    uint32             m_Flags;    //!< Option flags.
public:
#ifdef _WIN32
    typedef void* TAPDevice;
#else
    typedef struct ifreq    TAPDevice;
#endif
...
private:
     ...
     */
    void createTapDevice(TAPDevice& ifr, const std::string& devname, bool persistent);
     ...
     *  \param ip                   the ip that is going to be assigned to this TAP device.
     */
    void setIP(TAPDevice& ifr, int& sock, uint32 ip);
     ...
     */
    void start(TAPDevice& ifr, int sock);
private:
